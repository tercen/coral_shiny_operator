...
ui <- dashboardPage(
  ...
  dashboardHeader(
    title = tags$a(span(
      img(
        src = "logos/coral-logo-white2.png",
        height = 60,
        align = "left"
      )
    ), href = "http://phanstiel-lab.med.unc.edu/CORAL/"),
    titleWidth = 600,
            ...
            fluidRow(
              ...
              box(
                ...
                conditionalPanel(
                  ...
                  prettyCheckbox(
                    ...
                    label = "load default kinase groups",
                    value = FALSE,
                    shape = "round",
                    status = "primary"
                  ),
                ...
                # if Quantitative
                conditionalPanel(
                  condition = "input.branchcolortype == 'Quantitative' || input.branchcolortype == 'Tercen'",
                  prettyCheckbox(
                  ...
                  tags$br(),
                  prettyRadioButtons(
                    inputId = "BranchValueMissingKinase",
                    label = "Color Missing Kinases",
                  ...
                  conditionalPanel(
                    ...
                    colourInput(
                      inputId = "BranchValueMissingKinaseColor",
                      "Missing Kinase Color",
                      value = BG_col1,
                      showColour = "both"
                    )
                  )
              ...
              box(
                ...
                selectInput(
                  ...
                  choices = c("None", "Uniform", "Manual", 
                              "Categorical", "Quantitative", "Tercen"),
                  multiple = FALSE,
                  selected = "None",
                  width = "100%"
                ),
                ...
                # if Quantitative
                conditionalPanel(
                  condition = "input.nodecolortype == 'Quantitative' || input.nodecolortype == 'Tercen'",
                  prettyCheckbox(
                  ...
                  div(
                    prettyRadioButtons(
                      inputId = "NodeValueMissingKinase",
                      label = "Color Missing Kinases",
                      choices = c("automatically", "manually"),
                      selected = "automatically",
                      inline = TRUE
  ...
  server <- shinyServer(function(input, output, session) {
    ...
    outputjson      = tempfile(pattern = "kinome_tree",
                               tmpdir = "www/json",
                               fileext = ".json") # session specific json file describing network
    outputjsonshort = paste("json/", strsplit(outputjson, split = "/")[[1]][3], sep =
                              "") # used to communicate to js
    subdffile       = tempfile(pattern = "subdf",
                               tmpdir = "tempfiles",
                               fileext = ".txt") # temp file used to make json file
    ...
    insertUI(selector = "#treediv",
             where = "afterEnd",
             ui = source("R/renderTree.R", local = TRUE)$value)
    insertUI(
      selector = "#circlediv",
      where = "afterEnd",
      ui = div(
    ...
    replaceinfobox = function(infoboxcode)
    {
      ...
      insertUI(selector = "#InfoBox",
               where = "afterEnd",
               ui = source(infoboxcode, local = TRUE)$value)
    }
    ...
    observe({
      if (input$dashboardchooser == "Info")
      {
        ...
        updateRadioGroupButtons(session, inputId = "dashboardchooser2", selected =
                                  "Info")
      }
    })
    ...
    observe({
      if (input$dashboardchooser2 == "Plot")
      {
        ...
        updateRadioGroupButtons(session, inputId = "dashboardchooser", selected =
                                  "Plot")
      }
    })
    ...
    observe({
      ...
      {
        kinasegroupinfobr = paste(apply(
          data.frame(
            svginfo$dataframe$id.coral,
            svginfo$dataframe$kinase.group
          ),
          1,
          paste,
          collapse = "\t"
    ...
    newdf <- reactive({
      ...
      if (input$branchcolortype == "Manual")
      {
        ...
        if (length(selkinases) > 0)
        {
          ...
          selkinasesconverted = convertID (tempdf,
                                           kinasestoconvert,
                                           inputtype = input$branchManualIDtype)
        ...
        tempdf$branch.col =  color.by.selected(
          df = tempdf,
          sel = selkinasescoral,
          bg.col  = input$col_select_bg,
          sel.col = input$col_select
        )
        ...
        lines_and_offset = build.group.legend(
          yoffset = yoffset,
          groupslabels = c(
            input$branch_select_label,
            input$branch_nonselect_label
          ),
          groupcolors = c(input$col_select, input$col_select_bg),
          elementtype = "Branch",
          fontfamily = input$fontfamilyselect
        )
      ...
      if (input$branchcolortype == "Categorical")
      {
        ...
        if (input$branchgroupcolorpalettetype == "manual")
        {
          branchgroupcolpalette = c(
            input$branchgroupcol1,
            input$branchgroupcol2,
            input$branchgroupcol3,
            input$branchgroupcol4,
            input$branchgroupcol5,
            input$branchgroupcol6,
            input$branchgroupcol7,
            input$branchgroupcol8,
            input$branchgroupcol9,
            input$branchgroupcol10,
            input$branchgroupcol11,
            input$branchgroupcol12
          )
        }
        ...
        if (nrow(recolordf) > 0)
        {
          ...
          newcolors_and_colormapping = color.by.group(
            df = tempdf,
            recolordf = recolordf,
            bg.col = input$defaultbranchcolor_categorical,
            colors  = branchgroupcolpalette,
          ...
          lines_and_offset = build.group.legend(
            yoffset = yoffset,
            groupslabels = names(branch.group.colormapping),
            groupcolors = branch.group.colormapping,
            elementtype = "Branch",
            fontfamily = input$fontfamilyselect
          )
          lines = lines_and_offset[[1]]
      ...
      if (input$branchcolortype == "Quantitative" || input$branchcolortype == "Tercen")
      {
        # read in text area input
        if (input$branchcolortype == "Quantitative") {
          ...
          data <- getValues()
          recolordf <-
            data.frame(data$data[, c("rowNames", "Mean Kinase Statistic")],
                       stringsAsFactors = F)
        }
        ...
        if (nrow(recolordf) > 0)
        {
          ...
          {
            branchcolpalette = colorRampPalette(c(
              input$branch3col_low,
              input$branch3col_med,
              input$branch3col_hi
            ))(11)
          ...
          newcolors_and_colormapping = color.by.value(
            ...
            heatrange = c(input$minheat, input$maxheat),
            bg.col = bg.col
          )
          tempdf$branch.col = newcolors_and_colormapping[[1]]
          ...
          lines_and_offset = build.value.legend(
            yoffset = yoffset,
            minval = input$minheat,
            maxval = input$maxheat,
            palette = branchcolpalette,
            elementtype = "Branch",
            fontfamily = input$fontfamilyselect,
            subtitle = input$quantvaluenamebranchcolor
          )
          lines = lines_and_offset[[1]]
          yoffset = lines_and_offset[[2]] + 14
          legend = c(legend, lines)
      ...
      if (input$nodecolortype == "Manual")
      {
        ...
        if (length(selkinases) > 0)
        {
          ...
          selkinasesconverted = convertID (tempdf,
                                           kinasestoconvert,
                                           inputtype = input$NodeManualIDtype)
          if (nrow(selkinasesconverted) > 0)
          {
            selkinasescoral = selkinasesconverted[, 1]
          }
        ...
        tempdf$node.col =  color.by.selected(
          df = tempdf,
          sel = selkinasescoral,
          bg.col  = input$col_node_bg,
          sel.col = input$col_sel_node
        )
        
        # # build legend for Node Color (Manual Selection)
        lines_and_offset = build.group.legend(
          yoffset = yoffset,
          groupslabels = c(input$node_select_label, input$node_nonselect_label),
          groupcolors = c(input$col_sel_node, input$col_node_bg),
          elementtype = "Node",
          fontfamily = input$fontfamilyselect
        )
        lines = lines_and_offset[[1]]
      ...
      if (input$nodecolortype == "Categorical")
      {
        ...
        if (input$nodegroupcolorpalettetype == "manual")
        {
          nodegroupcolpalette = c(
            ...
            input$nodegroupcol8,
            input$nodegroupcol9,
            input$nodegroupcol10,
            input$nodegroupcol11,
            input$nodegroupcol12
          )
        }
        ...
        if (nrow(recolordf) > 0)
        {
          ...
          newcolors_and_colormapping = color.by.group(
            df = tempdf,
            recolordf = recolordf,
            bg.col = input$defaultnodecolor_categorical,
            colors  = nodegroupcolpalette,
            categories = categories
          )
          ...
          lines_and_offset = build.group.legend(
            yoffset = yoffset,
            groupslabels = names(node.group.colormapping),
            groupcolors = node.group.colormapping,
            elementtype = "Node",
            fontfamily = input$fontfamilyselect
          )
          lines = lines_and_offset[[1]]
      ...
      # color nodes Quantitative
      if (input$nodecolortype == "Quantitative" || input$nodecolortype == "Tercen")
      {
        # read in text area input
        if (input$nodecolortype == "Quantitative") {
          ...
          data <- getValues()
          recolordf <-
            data.frame(data$data[, c("rowNames", "Mean Final Score")],
                       stringsAsFactors = F)
        }
        ...
        if (nrow(recolordf) > 0)
        {
          ...
          {
            nodecolpalette = colorRampPalette(c(
              input$node3col_low,
              input$node3col_med,
              input$node3col_hi
            ))(11)
          ...
          newcolors_and_colormapping = color.by.value(
            ...
            heatrange = c(input$nodeminheat, input$nodemaxheat),
            bg.col = bg.col
          )
          tempdf$node.col = newcolors_and_colormapping[[1]]
          ...
          lines_and_offset = build.value.legend(
            yoffset = yoffset,
            minval = input$nodeminheat,
            maxval = input$nodemaxheat,
            palette = nodecolpalette,
            elementtype = "Node",
            fontfamily = input$fontfamilyselect,
            subtitle = input$quantvaluenamenodecolor
          )
          lines = lines_and_offset[[1]]
          yoffset = lines_and_offset[[2]] + 14
          legend = c(legend, lines)
      ...
      if (input$nodesizetype == "Quantitative")
      {
        ...
        if (nrow(resizedf) > 0)
        {
          radii_and_mapping = resizes.by.value(
            df = tempdf,
            resizedf = resizedf,
            sizerange = input$nodesizeValueslider,
            controlledrange = input$Manuallysetdatarange,
            minvalue = input$nodesizevaluemin,
            maxvalue = input$nodesizevaluemax,
            showall = input$nodesizefornotprovidedquantitative
          )
          ...
          lines_and_offset = build.nodesize.legend (
            ...
            minsize = input$nodesizeValueslider[1] ,
            maxsize = input$nodesizeValueslider[2],
            fontfamily = input$fontfamilyselect,
      ...
      if (input$fontcolorselect == "Manual")
      {
        ...
        if (length(selkinases) > 0)
        {
          ...
          selkinasesconverted = convertID (tempdf,
                                           kinasestoconvert,
                                           inputtype = input$labelsManualIDtype)
          if (nrow(selkinasesconverted) > 0)
          {
            selkinasescoral = selkinasesconverted[, 1]
          }
      ...
      if (input$nodestrokecolselect == "Selected")
      {
        ...
        if (length(kinases) > 0)
        {
          ...
          {
            # set colors based on selected ids
            tempdf$node.strokecol =  color.by.selected(
              df = tempdf,
              sel = selectedkinasesforstroke[, 1],
              bg.col  = input$NodeStrokeSelect_BG,
              sel.col = input$NodeStrokeSelect_FG
            )
          }
        }
      }
    ...
    output$plot1  <- renderSvgPanZoom ({
      ...
      writekinasetree(
        ...
        font = input$fontfamilyselect,
        labelselect = input$kinaselabelselect,
        groupcolor = input$groupcolorchoose
      )
    ...
    output$circlelayout <- reactive({
      ...
      makejson(
        ...
        output = outputjson,
        BGcol = BG_col1,
        BGstrolecol = BGstrolecol,
        colsubnodes = input$colorsubnodes,
        labelselect = input$kinaselabelselect,
        defaultnoderadius = input$size_node_single,
        legend = svginfo$legend,
        xshift = 80,
        yshift = 60
      )
    ...
    output$KinaseTable <- DT::renderDataTable({
      ...
      rgbcolors <- apply(grDevices::col2rgb(mycolors), 2,
                         function(rgb)
                           sprintf("rgb(%s)", paste(rgb, collapse = ",")))
      tgtbranch <-
        sprintf('<span style="color:%s">&#9608;</span>', rgbcolors)
